...
record RegistroDto(Long id, String idPacienteRegional, String cedula, LocalDate fecha, String sede, String medico, String especialidad, String tipoConsulta, String diagnostico, String tratamiento, String motivo, String evolucion, String tipoExamen, String resultado, String observaciones, String tipoEstudio, String formato, String url, String regionAnatomica, String estado, String tecnicoResponsable, LocalTime hora, LocalDateTime fechaSolicitud, LocalDateTime fechaRealizacion) {}
record RegistroRequest(@NotBlank String cedula, String idPacienteRegional, @NotNull LocalDate fecha, @NotBlank String sede, String medico, String especialidad, String tipoConsulta, String diagnostico, String tratamiento, String motivo, String evolucion, String tipoExamen, String resultado, String observaciones, String tipoEstudio, String formato, String url, String regionAnatomica, String estado, String tecnicoResponsable, String hora) {}
...
class RegistroController {
  ...
  @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE) @ResponseStatus(HttpStatus.CREATED) @PreAuthorize("hasAnyRole('ADMIN','MEDICO','IMAGENOLOGIA')") RegistroDto crearConArchivo(
    ...
    return service.crear(new RegistroRequest(cedula, idPacienteRegional, LocalDate.parse(fecha), sede, medico, null, null, null, null, null, null, null, resultado, observaciones, tipoEstudio, formato, url, regionAnatomica, estado, tecnicoResponsable, hora), http);
  }
...
class RegistroService {
  ...
  RegistroDto crear(RegistroRequest r, HttpServletRequest http) { if (r.fecha().isAfter(LocalDate.now())) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "La fecha no puede ser futura."); RegistroDto dto = repo.crear(r); Auditoria.registrar(repo.jdbc(), "SOLICITAR_IMAGENOLOGIA", dto.idPacienteRegional(), http); return dto; }
...
class RegistroRepository {
  ...
  RegistroDto crear(RegistroRequest r) {
    ...
    jdbc.update("INSERT INTO estudios_imagenologia(id_paciente_regional,cedula,fecha,sede,medico,especialidad,tipo_consulta,diagnostico,tratamiento,motivo,evolucion,tipo_examen,resultado,observaciones,tipo_estudio,formato,url,region_anatomica,estado,tecnico_responsable,hora,fecha_solicitud,fecha_realizacion) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", normalizarPaciente(r),r.cedula(),r.fecha().toString(),r.sede(),r.medico(),r.especialidad(),r.tipoConsulta(),r.diagnostico(),r.tratamiento(),r.motivo(),r.evolucion(),r.tipoExamen(),r.resultado(),r.observaciones(),r.tipoEstudio(),r.formato(),r.url(),r.regionAnatomica(),estado,r.tecnicoResponsable(),normalizarHora(r.hora()),LocalDateTime.now().toString(),fechaRealizacion);
  ...
  RegistroDto editar(Long id, RegistroRequest r) {
    String estado = normalizarEstado(r.estado(), r.url(), r.resultado());
    String fechaRealizacion = "REALIZADO".equals(estado) || "INFORMADO".equals(estado) ? LocalDateTime.now().toString() : null;
    int rows = jdbc.update("UPDATE estudios_imagenologia SET id_paciente_regional=?,cedula=?,fecha=?,sede=?,medico=?,especialidad=?,tipo_consulta=?,diagnostico=?,tratamiento=?,motivo=?,evolucion=?,tipo_examen=?,resultado=?,observaciones=?,tipo_estudio=?,formato=?,url=?,region_anatomica=?,estado=?,tecnico_responsable=?,hora=?,fecha_realizacion=COALESCE(?,fecha_realizacion) WHERE id=?", normalizarPaciente(r),r.cedula(),r.fecha().toString(),r.sede(),r.medico(),r.especialidad(),r.tipoConsulta(),r.diagnostico(),r.tratamiento(),r.motivo(),r.evolucion(),r.tipoExamen(),r.resultado(),r.observaciones(),r.tipoEstudio(),r.formato(),r.url(),r.regionAnatomica(),estado,r.tecnicoResponsable(),normalizarHora(r.hora()),fechaRealizacion,id);
  ...
  RegistroDto registrarResultado(Long id, RegistroRequest r) {
    int rows = jdbc.update("UPDATE estudios_imagenologia SET resultado=?, observaciones=?, formato=?, url=?, estado=?, tecnico_responsable=?, hora=?, fecha_realizacion=? WHERE id=?", r.resultado(), r.observaciones(), r.formato(), r.url(), normalizarEstado(r.estado() == null ? "REALIZADO" : r.estado(), r.url(), r.resultado()), r.tecnicoResponsable(), normalizarHora(r.hora()), LocalDateTime.now().toString(), id);
    if (rows == 0) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Registro no encontrado.");
    return obtener(id).orElseThrow();
  }
  ...
  RegistroDto map(java.sql.ResultSet rs, int row) throws java.sql.SQLException { return new RegistroDto(rs.getLong("id"),rs.getString("id_paciente_regional"),rs.getString("cedula"),LocalDate.parse(rs.getString("fecha")),rs.getString("sede"),rs.getString("medico"),rs.getString("especialidad"),rs.getString("tipo_consulta"),rs.getString("diagnostico"),rs.getString("tratamiento"),rs.getString("motivo"),rs.getString("evolucion"),rs.getString("tipo_examen"),rs.getString("resultado"),rs.getString("observaciones"),rs.getString("tipo_estudio"),rs.getString("formato"),rs.getString("url"),rs.getString("region_anatomica"),rs.getString("estado"),rs.getString("tecnico_responsable"),parseTime(rs.getString("hora")),parseDateTime(rs.getString("fecha_solicitud")),parseDateTime(rs.getString("fecha_realizacion"))); }
}
